...
from molecular_constraints.models import ALLOWED_PROPERTIES, PropertyConstraint
...
# Group 1: optional leading number (for "500>=MW" style, not required by spec
#           but falls out naturally)
# We handle two sub-patterns:
#   name OP number   →  property on left
#   number OP name   →  property on right (flipped semantics)
...
def _apply_operator(
    ...
    The operator semantics follow mathematical convention:
      name < val   →  max_val = val  (exclusive, but PropertyConstraint
                       stores inclusive bounds — we subtract/add nothing;
                       the caller interprets < vs <= at evaluation time.
                       For this parser, < sets max_val and > sets min_val,
                       matching the PropertyConstraint's inclusive-bound model
                       because the ticket specifies window constraints, not
                       strict inequalities.)
    """
    # Mapping: how does the operator constrain the property?
    if op == "<" or op == "<=":
