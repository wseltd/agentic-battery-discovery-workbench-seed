...
Parses a raw dict (matching the user-facing constraint schema from Q10/Q2)
into structured ParsedConstraints with typed NumericRange objects, validated
SMARTS lists, and element allow/exclude lists.  Raises ConstraintParseError
with descriptive messages on malformed input rather than propagating opaque
third-party tracebacks.

Chose a dict-keyed dispatch over a token-string grammar because the upstream
request schema already provides structured keys — re-parsing strings would
...
# Maps user-facing aliases to a canonical internal name so that both
# 'logP' and 'cLogP' resolve to the same constraint slot.
...
def parse_constraints(raw: dict) -> ParsedConstraints:
    ...
    Accepts a dict mapping property names to range specs (``{min, max}``),
    SMARTS pattern lists, and element symbol lists.  Validates all values
    eagerly so callers get clear errors at parse time rather than silent
    misbehaviour at evaluation time.

    Args:
        raw: Constraint dict from the user request.  Numeric properties
            use ``{"min": float, "max": float}`` (either bound optional).
            SMARTS keys use lists of pattern strings.  Element keys use
            lists of element symbol strings.
    ...
    Raises:
        ConstraintParseError: If any value is malformed — wrong type,
            impossible range (min > max), invalid SMARTS pattern, or
            unrecognised key.
    """
