...
Regex-based parser that recognises space group numbers, Hermann-Mauguin
symbols, and crystal-system names in free-form materials science queries.
No LLM calls — pure pattern matching.

Supported formats:
- Explicit number: ``"space group 225"``, ``"SG 62"``
- Hermann-Mauguin symbol: ``"Fm-3m"``, ``"Pnma"`` (looked up via
  caller-supplied ``symbol_to_number`` dict)
- Crystal-system name: ``"cubic"``, ``"tetragonal"`` etc.
"""
...
# Imported from constraints.py at the module level would create a circular
# concern — instead, this is the canonical definition shared via import
# in constraints.py.  Defined here because this module owns the
# crystal-system → SG-range mapping logic.
...
# Precompiled regex: "space group 62", "SG 62", "spacegroup 62"
...
def parse_space_group(
    ...
    1. Explicit numeric references (``"space group 225"``, ``"SG 62"``).
       Numbers outside 1–230 are silently ignored.
    2. Hermann-Mauguin symbols (``"Fm-3m"``, ``"Pnma"``), resolved via
       *symbol_to_number*.  Longer symbols are tried first so that
       ``"Fm-3m"`` matches before ``"Fm-3"``.
    3. Crystal-system names (``"cubic"``, ``"tetragonal"``), which yield
       the system name but no single SG number.
    ...
    Returns:
        A 2-tuple ``(sg_number, crystal_system)``.  Either or both may be
        ``None`` if the corresponding pattern was not found.
    """
...
def _extract_sg_symbol(
    ...
    Sorting longest-first prevents partial matches: ``"Fm-3m"`` is checked
    before ``"Fm-3"`` or ``"F"`` so the most specific symbol wins.
    """
