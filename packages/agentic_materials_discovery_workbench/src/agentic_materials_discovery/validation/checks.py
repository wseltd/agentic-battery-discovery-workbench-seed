...
# Minimum credible unit-cell volume in A^3 -- anything below this
...
def validate_lattice_sanity(structure: Structure) -> MaterialsValidationResult:
    ...
    if volume < _MIN_LATTICE_VOLUME:
        return MaterialsValidationResult(
            passed=False,
            stage="lattice_sanity",
            message=f"Lattice volume {volume:.6g} A^3 is below minimum {_MIN_LATTICE_VOLUME}",
            severity="hard",
        )
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if d < threshold:
                ...
                violations.append(
                    f"{sym_i}(#{i})--{sym_j}(#{j}): {d:.4f} A < {threshold:.4f} A"
                )
