...
def _mol_to_inchikey(mol: Chem.Mol) -> str | None:
    ...
    inchi_val = MolToInchi(mol)
    if inchi_val is None:
        return None
    return InchiToInchiKey(inchi_val)
...
def compute_target_satisfaction(
    ...
    Args:
        ...
        constraints: Property name to ``(min, max)`` window.
...
def compute_internal_diversity(
    ...
    Returns:
        Tuple of (diversity_mean, diversity_std).
    """
    ...
    bit_counts = bit_matrix.sum(axis=1)
    dot_products = bit_matrix @ bit_matrix.T
    ...
    std_d = float(distances.std(ddof=0))
    return mean_d, std_d
